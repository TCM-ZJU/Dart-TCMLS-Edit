 ...
 * 
 */
public class QueryResultHtmlClinicOrderByUnit extends QueryResultHtmlFactory {
	...
	public String ShowResultWithPageNo(ArrayList<Lcyj> lcyjDwList, int pageNum) {
		...
		for (int counter = BeginPoint; counter < endPoint; counter++) {
			String DW = lcyjDwList.get(counter).getDW().toString();
			String[] dwStrList=DW.split("\\ ");
 			String DWMC=dwStrList[1].toString();
	...
	private ArrayList<Lcyj> generateLcyjmcList() {
		...
		ArrayList<Lcyj> lcyjmcList = lcyjDAO.GetDWmcByJbmc();
		return lcyjmcList;
	}
